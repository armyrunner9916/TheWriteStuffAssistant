...
const SongwritingHistory = () => {
  ...
  const categoryLabels = {
    ...
    songwriting_unified: 'Songwriting Assistant'
  };
    ...
    const fetchHistory = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from("query_history")
          .select("*")
          .in("query_type", songwritingCategories)
          .eq("user_id", user.id)
  ...
  const handleDownload = (entry) => {
    ...
    const title = `Songwriting_${new Date(entry.created_at).toLocaleDateString()}`;
  ...
  const renderHistory = (items) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {items.map((item) => {
         const isUnified = item.query_type === 'songwriting_unified';
  ...
  return (
    <>
      <Helmet>
        <title>Songwriting History | The Write Stuff</title>
        <meta name="description" content="Review your past generations from the Songwriting tools." />
      </Helmet>
      <div className="min-h-screen bg-black text-yellow-400 p-4 sm:p-6">
        <div className="max-w-7xl mx-auto">
          <header className="flex items-center justify-between mb-8">
            <Button onClick={() => navigate("/songwriting")} variant="outline" className="bg-black text-yellow-400 border-yellow-400 hover:bg-zinc-800">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back to Songwriting Tool
            </Button>
            <h1 className="text-3xl font-bold text-yellow-400">Songwriting History</h1>
            <div />
          </header>
          ...
          {loading ? (
            ...
            <div className="text-center text-yellow-400/70">
              <p>You have no history for the Songwriting section yet.</p>
            </div>
          ) : (
...
export default SongwritingHistory;
